...
import { Plus, Search, Loader2, ChevronRight, CheckCircle2, Trash2, AlertCircle } from "lucide-react";
...
export default function OperationsPage() {
  ...
  const deleteOp = useMutation({
    mutationFn: ({ type, id }: { type: string, id: number }) => api.delete(`${type}/${id}/`),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: [variables.type] });
      queryClient.invalidateQueries({ queryKey: ["dashboard"] });
      toast.success(`${variables.type.charAt(0).toUpperCase() + variables.type.slice(1, -1)} deleted`);
    },
    onError: () => toast.error("Failed to delete operation"),
  });

  const finalizeOp = useMutation({
    mutationFn: ({ type, id }: { type: string, id: number }) => api.post(`${type}/${id}/finalize/`),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: [variables.type] });
      queryClient.invalidateQueries({ queryKey: ["dashboard"] });
      queryClient.invalidateQueries({ queryKey: ["products"] });
      toast.success("Operation finalized and stock updated");
    },
    onError: (err: any) => toast.error(err.response?.data?.error || "Failed to finalize"),
  });
  ...
  if (isLoadingReceipts || isLoadingDeliveries || isLoadingTransfers) {
  ...
  const filterData = (data: any[]) => {
    ...
    return data.filter((item: any) => 
      ...
      (item.warehouse_name && item.warehouse_name.toLowerCase().includes(q)) ||
      (item.from_location_name && item.from_location_name.toLowerCase().includes(q)) ||
      (item.to_location_name && item.to_location_name.toLowerCase().includes(q)) ||
  ...
  const modalTitle = activeTab === "receipts" ? "Create Receipt" : activeTab === "deliveries" ? "Create Delivery" : "Create Transfer";

  const renderActions = (type: string, item: any) => (
    <div className="flex items-center gap-1">
      {item.status !== 'done' && (
        <button 
          onClick={() => finalizeOp.mutate({ type, id: item.id })}
          disabled={finalizeOp.isPending}
      ...
      <button 
        onClick={() => deleteOp.mutate({ type, id: item.id })}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-foreground">Inventory Operations</h1>
